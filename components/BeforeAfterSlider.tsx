...
const BeforeAfterSlider: React.FC<BeforeAfterSliderProps> = ({ beforeImage, afterImage }) => {
  ...
  return (
    <div 
      ref={containerRef}
      className="relative w-full aspect-[3/4] overflow-hidden select-none cursor-ew-resize group border border-neutral-800"
    ...
    >
      ...
      {/* Label After */}
      <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-md px-2 py-1 text-[10px] uppercase tracking-widest text-amber-500 font-bold z-10 pointer-events-none">
         AI Style
      </div>
      ...
      >
        <img 
          ...
          style={{ width: containerRef.current?.offsetWidth || '100%' }} // Ensure image doesn't squash
          draggable={false}
        />
        {/* Label Before */}
